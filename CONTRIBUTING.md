...
deliberately fixed: MCP, REST and a CLI over those Google Messages accounts. It is public because the
...
- **No feature requests.** Anything outside `plans/AGENT_GM_SPEC.md` §1
  ...
  human-facing chat client are all permanent non-goals. (Multi-*account* is
...
Development happens through Devbox only — see `plans/AGENT_GM_SPEC.md` §13.6. Run
`devbox run check` before proposing anything. Do not install tooling on the
host; add it to `devbox.json`.

Security issues: do not open a public issue. Email the address on the
