...
| `GET` | `/v1/accounts/{account_id}/events` | `messages:read` | **SSE**. It opens with an `account.state` snapshot of every account in scope — a client that has just connected holds no prior state, so the current state is the first thing it did not know, and a stream of nothing but changes is indistinguishable from a broken one. On a snapshot frame `from` is **null** — there is no previous state to name — and `state_reason` is null when there is none, per §4.7; neither is ever `""`. Then one `account.state_changed` per state change, plus a 30s heartbeat. The only streaming route; it carries no message data, so it needs no replay ring and no cursor. Backs `agm session --watch`. Omitting the ID (`/v1/accounts/events`) streams every account's changes, each tagged |
...
21. `connector/startchat.go` still sets `RCSGroupName` on the **first**
    `GetOrCreateConversation` (`:189`) and `CreateRCSGroup` with a non-nil
    name on the `CREATE_RCS` **retry** (`:214-218`). D34 is a deliberate
    divergence from the first half and keeps the second, and a divergence is
    only meaningful while the thing it diverges from is still true: a later
    pin that moved the name off the first call by itself would leave D34
...
2. `fixture-validation` passes all twenty-one assertions of §13.4 against a fresh
