...
// --- planted, skipped until the release script grows the two guards ------------
...
func TestPublishRefusesToRecordAnUnknownImageDigest(t *testing.T) {
	t.Skip("planted by the reviewer at a9f5626: do_publish does not guard the digest yet")
	sh := repoFile(t, "scripts/release.sh")
...
func TestOnlyASemverTagPublishes(t *testing.T) {
	t.Skip("planted by the reviewer at a9f5626: require_tag accepts any v-prefixed tag")
	sh := repoFile(t, "scripts/release.sh")
