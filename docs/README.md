...
**The approved-test-number rule.** Live sends go only to `+1<APPROVED_DIRECT_NUMBER>`
(direct) and `+1<APPROVED_GROUP_NUMBER_1>` / `+1<APPROVED_GROUP_NUMBER_2>` (group), and only the
